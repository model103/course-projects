...
def get_input():
    with open('query.txt', 'r') as qu:
        t_queries = len(qu.readlines())
        qu.seek(0.0)
        q = []
        for idx in range(t_queries):
            q.append(qu.readline().strip())
    acc = 0
    kb_obj = KnowledgeBase()
    with open('KnowledgeBase.txt', 'r') as KB:
        t_kb1 = len(KB.readlines())
        KB.seek(0.0)
        for idx in range(t_kb1):
            kb_obj.add_axiom(KB.readline().strip(), idx)
            acc = idx

    with open('instance.txt', 'r') as ins:
        t_kb2 = len(ins.readlines())
        ins.seek(0.0)
        for idx in range(t_kb2):
            kb_obj.add_axiom(ins.readline().strip(), idx+acc)

    return t_queries, q, t_kb1+t_kb2, kb_obj, idx+acc
...
def substitute(sentence, query, substitution, pred_query, pred_sentence):
        ...
        for sentence in kb:
            ...
            for pred_sentence in predicates_sentence:
                ...
                if (pred_query_name == '~' + pred_sentence_name or pred_sentence_name == '~' + pred_query_name) \
                    ...
                    if substitution != False:
                        answer = substitute(sentence, query, substitution, pred_query, pred_sentence)
